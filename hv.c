	       ...
	       int flags, int action, SV *val, U32 hash)
{
    ...
    if (oentry->hent_hash != hash) {
        entry = oentry->hent_he->hent_next;
        if (!entry)
            goto not_found;
    }
	...
	if (flags & HVhek_FREEKEY)
        ...
        DEBUG_H(S_assert_hechain(aTHX_ oentry->hent_he));
#endif
	if (return_svp) {
		   ...
		   int k_flags, I32 d_flags, U32 hash)
{
	...
	if (mro_changes == 1)
        ...
        DEBUG_H(S_assert_hechain(aTHX_ first_entry->hent_he));
#endif
	return sv;
    }
  ...
  not_found:
	...
	Safefree(key);
    DEBUG_H(PerlIO_printf(Perl_debug_log, "HASH %6u\t%6u\t%u DEL-\t{%.*s}\n",
                          (unsigned)HvTOTALKEYS(hv), (unsigned)HvMAX(hv), linear, (int)klen, key));
    return NULL;
}
...
S_hsplit(pTHX_ HV *hv, U32 const oldsize, U32 newsize)
{
    ...
    Zero(&a[oldsize * sizeof(AHE)], (newsize-oldsize) * sizeof(AHE), char);
	...
	while (entry) {
            U32 j = HvHASH_INDEX(ahe->hent_hash, newsize);
	    if (j != i) {
                AHE *nhe = &aep[j];
                ahe->hent_he = HeNEXT(entry);
...
Perl_newHVhv(pTHX_ HV *ohv)
{
	...
	for (; oent <= last; oent++, ents++) {
	    ...
	    for (prev = NULL, e = oent->hent_he; e; e = HeNEXT(e)) {
		const U32 hash   = HeHASH(e);
		const char * const key = HeKEY(e);
		const I32 len    = HeKLEN(e);
		const int flags  = HeKFLAGS(e);
		HE * const ent   = new_HE();
...
Perl_hfree_next_entry(pTHX_ HV *hv, U32 *indexp)
{
    ...
    array = HvARRAY(hv);
    assert(array);
    while ( ! ((entry = array[*indexp].hent_he)) ) {
	if (++(*indexp) > HvMAX(hv))
	    *indexp = 0;
#ifdef PERL_PERTURB_KEYS_TOP
    ...
    array[*indexp].hent_he = HeNEXT(entry);
    array[*indexp].hent_hash = HeHASH(entry);
    ((XPVHV*) SvANY(hv))->xhv_keys--;
...
S_share_hek_flags(pTHX_ const char *str, I32 len, U32 hash, int flags)
{
    ...
    */
	...
	HEK *hek;
	char *k;
	AHE *const head = &HvARRAY(PL_strtab)[hindex];
	HE *const next = head->hent_he;
