 ...
 * constant folded.
 * max 10 ops, no new lexicals, no intermediate return, no recursion, ...
 */
...
S_cv_do_inline(pTHX_ OP *o, OP *cvop, CV *cv)
{
    ...
    if (meth) { /* push self */
        ...
        if (OP_TYPE_IS(o->op_next, OP_CONST)) { /* pkg->meth yes, if pkg::meth exists */
            ...
            op_free(pushmarkop);
            o->op_next = on;
            pushmarkop = cLISTOPx(o)->op_first;
    ...
    if (arg->op_next != cvop) { /* has args */
        ...
        cLISTOPx(list)->op_last = o; /* XXX this list might be too long still re siblings */
    ...
    if (!o->op_next || !with_enter_leave ) { /* no LEAVE, so no ENTER also */
        ...
        OP *o = arg->op_next; /* the entersub */
        if (list)
            list->op_next = o;
        else
            firstop->op_next = o;
        cUNOPo->op_first = 0; /* protect cv from being freed */
